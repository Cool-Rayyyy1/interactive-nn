import * as math from '$lib/math/activation';
...
// Mapping for Neuron labels in Latex
...
export class NeuralNetworkModel {
  // Svelte 5 state properties using runes
  ...
  isTraining = $state<boolean>(false);
  ...
  constructor(learningRate: number, target: number, maxEpoch: number, activationType: ActivationType, x1: number, x2: number) {
    ...
    this.neuronValues['i0'] = { val: x1, raw: x1, error: 0 };
    this.neuronValues['i1'] = { val: x2, raw: x2, error: 0 };
  }

  // Update dynamic values
  updateConfig(learningRate: number, target: number, maxEpoch: number, activationType: ActivationType, x1: number, x2: number) {
    this.learningRate = learningRate;
    this.target = target;
    this.maxEpoch = maxEpoch;
    this.activationType = activationType;
    if (this.x1 !== x1 || this.x2 !== x2) {
      ...
      if (this.step === 'idle' && !this.isTraining) {
        this.neuronValues['i0'].val = x1;
        this.neuronValues['i0'].raw = x1;
        this.neuronValues['i1'].val = x2;
        this.neuronValues['i1'].raw = x2;
      }
    }
  }
  ...
  forwardStep(index: number) {
    ...
    const activeVal = this.activationType === 'sigmoid' ? math.sigmoid(sum) : this.activationType === 'tanh' ? math.tanh(sum) : math.relu(sum);
    
    this.neuronValues[targetId] = {
      raw: sum,
      val: activeVal,
      error: this.neuronValues[targetId].error
    };
    ...
    this.numericMath = `${targetLabel} = ${funcName}(${numericStr}) = ${activeVal.toFixed(3)}`;
    this.mathDetails = `Σ = ${mathStr} = ${sum.toFixed(3)}\na = f(Σ) = ${activeVal.toFixed(3)}`;
  }
  ...
  backwardStep(index: number) {
    ...
    const sDeriv = this.activationType === 'sigmoid' ? math.sigmoidDeriv(z) : this.activationType === 'tanh' ? math.tanhDeriv(z) : math.reluDeriv(z);
  ...
  updateStep(index: number) {
    ...
    if (index < weightKeys.length) {
      ...
      const grad = this.gradients.weights[k] || 0;
      ...
      if (!k) return;
      const grad = this.gradients.biases[k] || 0;
  ...
  handleStep() {
    ...
    if (this.step === 'idle') {
      ...
      if (this.subStep < 5) {
        ...
        this.neuronValues.o0.error = o0_error;
        ['h2_0', 'h2_1'].forEach(hId => {
          this.gradients.weights[`${hId}-o0`] = o0_error * this.neuronValues[hId].val;
        });
        this.gradients.biases['o0'] = o0_error;
      ...
      if (this.subStep < 14) {
        ...
        this.history.push({ epoch: this.epoch + 1, loss });
